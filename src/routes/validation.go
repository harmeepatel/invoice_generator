...
func Validate(vg *bunrouter.Group) {
	...
	vg.GET("/all", func(w http.ResponseWriter, req bunrouter.Request) error {
		...
		igstRate, _ := strconv.ParseFloat(req.Form.Get("igst"), 64)

		model.ActiveInvoice.Customer = &model.Customer{
		...
		model.ActiveInvoice.IgstRate = igstRate

		gst, _ := strconv.ParseFloat(req.Form.Get("gst"), 64)
		qty, _ := strconv.Atoi(req.Form.Get("quantity"))
		rate, _ := strconv.ParseFloat(req.Form.Get("rate"), 64)
		discount, _ := strconv.ParseFloat(req.Form.Get("discount"), 64)
		model.ActiveItem = &model.InvoiceItem{
			Quantity:     qty,
			Rate:         rate,
			Discount:     discount,
			Gst:          gst,
